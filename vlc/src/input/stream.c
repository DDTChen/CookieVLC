...
static int AStreamReadStream( stream_t *s, void *p_read, unsigned int i_read )
{
    msg_Dbg( s, "#### AStreamReadStream i_read=%d", i_read );
    stream_sys_t *p_sys = s->p_sys;
...
static int AStreamReadNoSeekStream( stream_t *s, void *p_read, unsigned int i_read )
{
msg_Dbg( s, "#### AStreamReadNoSeekStream " );
    stream_sys_t *p_sys = s->p_sys;
    stream_track_t *tk = &p_sys->stream.tk[p_sys->stream.i_tk];
...
msg_Dbg( s, "#### AStreamReadNoSeekStream 0" );
        return 0; /* EOF */
    }
...
msg_Dbg( s, "#### before AStreamReadNoSeekStream AStreamRefillStream" );
            if( AStreamRefillStream( s ) )
            {
                /* EOF */
                if( tk->i_start >= tk->i_end ) break;
            }
msg_Dbg( s, "#### after AStreamReadNoSeekStream AStreamRefillStream " );
        }
    }
msg_Dbg( s, "#### before AStreamReadNoSeekStream return" );
    return i_data;
}
...
{
        msg_Dbg( s, "#### stream_Read ... ..." );
//int real_read = s->pf_read( s, p_read, i_read );
//for (int i=0; i<real_read; i++) {
    //msg_Dbg( s, "#### OpenDemux data: %X", *(uint8_t *)(p_read + i * sizeof(uint8_t)) );
//    *(uint8_t *)(p_read + i * sizeof(uint8_t)) = ~ *(uint8_t *)(p_read + i * sizeof(uint8_t));
//    msg_Dbg( s, "#### OpenDemux data: %X", *(uint8_t *)(p_read + 0 * sizeof(uint8_t)) );
//}
//    return real_read;//s->pf_read( s, p_read, i_read );
