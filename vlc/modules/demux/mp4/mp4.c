...
{
msg_Dbg( p_demux, "#### UpdateSeekpoint" );
    demux_sys_t *p_sys = p_demux->p_sys;
    int64_t i_time;
    int i;
    if( !p_sys->p_title )
...
static void LoadChapterApple( demux_t  *p_demux, mp4_track_t *tk )
{
    ...
    for( tk->i_sample = 0; tk->i_sample < tk->i_sample_count; tk->i_sample++ )
    {
        ...
        {
            char p_buffer[256];
msg_Dbg( p_demux, "#### Before stream_Read 1" );
...
static int MP4_frg_GetChunk( demux_t *p_demux, MP4_Box_t *p_chunk, unsigned *i_tk_id )
{
    ...
    *i_tk_id = i_track_ID;
    ...
    /* Skip header of mdat */
msg_Dbg( p_demux, "#### Before stream_Read 2" );
    ...
    for( uint32_t i = 0; i < ret->i_sample_count; i++)
    {
        ...
        if( ret->p_sample_data[i] == NULL )
            return VLC_ENOMEM;
msg_Dbg( p_demux, "#### Before stream_Read 3" );
...
msg_Dbg( p_demux, "#### begin DemuxFrg" );
    /* check for newly selected/unselected track */
    ...
    for( i_track = 0; i_track < p_sys->i_tracks; i_track++ )
    {
        ...
        {
msg_Dbg( p_demux, "#### before MP4_frg_GetChunks" );
