...
static vlc_array_t* get_cookie_array( access_t *p_access, const char * urlQueryString );
...
static int Open( vlc_object_t *p_this )
{
    ...
    msg_Dbg( p_access, "#### http cookie before get_cookie_array: %s ", queryString);
    vlc_array_t* cookies = get_cookie_array( p_access, queryString );

    queryString[0] = '\0';

    if (vlc_array_count( cookies ) > 0) {
    msg_Dbg( p_access, "#### http cookie vlc_array_count( cookies ) > 0");
        return OpenWithCookies( p_this, p_access->psz_access, 5, cookies );
    } else {
    msg_Dbg( p_access, "#### http cookie vlc_array_count( cookies ) == 0");
        return OpenWithCookies( p_this, p_access->psz_access, 5, NULL );
    }
                            ...
                            unsigned i_redirect, vlc_array_t *cookies )
{
    ...
    msg_Dbg( p_access, "#### http cookie OpenWithCookies location: %s ", p_access->psz_location);
    msg_Dbg( p_access, "#### http cookie ??? count: %d ", vlc_array_count( saved_cookies ));
    if (vlc_array_count( saved_cookies ) > 0) {
        for (int i=0;i<vlc_array_count( saved_cookies );i++) {
            msg_Dbg( p_access, "#### http cookie: %s ", vlc_array_item_at_index( saved_cookies, i ));
        }
    }
...
//msg_Dbg( p_access, "#### 1-1 http *p_buffer");
//*p_buffer =  ~ (*p_buffer);
//msg_Dbg( p_access, "#### 1-2 *p_buffer='%X'", *p_buffer);
    return i_read;

fatal:
//msg_Dbg( p_access, "#### 2-1 http *p_buffer");
//*p_buffer = (*p_buffer);
//msg_Dbg( p_access, "#### 2-2 *p_buffer='%X'", *p_buffer);
...
static ssize_t ReadCompressed( access_t *p_access, uint8_t *p_buffer,
                               size_t i_len )
{
msg_Dbg( p_access, "#### ReadCompressed");
    access_sys_t *p_sys = p_access->p_sys;
...
static int Request( access_t *p_access, uint64_t i_tell )
{
    ...
    msg_Dbg( p_access, "#### http cookie psz_cookie_content: %s ", psz_cookie_content);


        net_Printf( p_access, p_sys->fd, pvs, "Cookie: close\r\n" );
            ...
            //{
                ...
                if( net_Printf( p_access, p_sys->fd, pvs, "Cookie: %s\r\n", psz_cookie_content ) < 0 ) {
                    msg_Err( p_access, "#### http cookie failed to send Cookie" );
                } else {
                   msg_Dbg( p_access, "#### http cookie send Cookie success");
                }
   ...
   // msg_Dbg( p_access, "#### http cookie Cookie: %s ", vlc_array_item_at_index( p_sys->cookies, 1));

   //     net_Printf( p_access, p_sys->fd, pvs, "Cookie: %s", vlc_array_item_at_index( p_sys->cookies, 1));
    }
...
static vlc_array_t* get_cookie_array( access_t *p_access, const char * urlQueryString )
{
    vlc_array_t * cookies = vlc_array_new();

    if ( urlQueryString == NULL ) return cookies;

msg_Dbg( p_access, "#### http cookie urlQueryString: %s ", urlQueryString);

    char * queryString = strdup( urlQueryString );
    if( !queryString ) return cookies;
msg_Dbg( p_access, "#### http cookie queryString: %s ", queryString);
    char * searchResult;
    while (queryString) {
        searchResult = strstr(queryString, "; ");
        if (searchResult == NULL && queryString != "") {
                vlc_array_append(cookies, queryString+1);   
            break;
        } else if (searchResult != "; ") {
            vlc_array_append(cookies, searchResult+2);
        }
        searchResult[0] = '\0';
msg_Dbg( p_access, "#### http cookie searchResult: %s ", queryString);
    }
    return cookies;
}
